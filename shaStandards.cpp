...
#define vectorLength 9161
// std::string inputs[vectorLength] = {"abc","hello","world","test","async","pipeline",
//     "sha256","dualrail","handshake","token","stall","buffer","signal",
//     "timing","gate","logic","round","compress","message","digest","crypto",
//     "hash","latency","throughput","elastic","micropipe","request",
//     "acknowledge","valid","ready","compute","network","system","thread",
//     "packet","channel","storage","vector","matrix","binary","hexdata",
//     "transfer","control","backpress","nullwave","spacer","bundle","encoded",
//     "decoder","encoder","transmit","receive","forward","reverse","monitor",
//     "observe","trigger","propagate","feedback","evaluate","iterate",
//     "schedule","process","runtime","compile","execute","compare","analyze",
//     "measure","profile","testing","simulate","hardware","software","parallel",
//     "serial","synchronous","clockless","delayline","arbiter","latch",
//     "flipflop","register","datapath","bitstream","wordfill","constant",
//     "entropy","security","checksum","compressor","validator","generator",
//     "sequencer","controller","allocator","multiplex","demultiplex",
//     "crossbar","interface"};
...
// WILL NOT BE PRESENT IN ASYNC
// Rotate right
uint32_t rR(uint32_t x, uint32_t n)
{
    n &= 31;
    return (x >> n) | (x << (32 - n));
}
...
// WILL NOT BE PRESENT IN ASYNC
// Right shift
uint32_t rS(uint32_t x, uint32_t n)
{
    return x >> n;
}

// Padding
std::vector<uint8_t> padInput(const std::vector<uint8_t>& data)
{
    // The message is padded to resemble this format
    // Message L bits long + 1 + K 0s + L as 64bit int
        // L is length of message in bits
        // K is number of 0s such that padded message length %512 = 0

    uint64_t L = static_cast<uint64_t>(data.size()) * 8; // # *8 for bits
    ...
    // Adds 1 byte of 0s until formula above is resolved
    while (((msg.size() * 8) + 64) % 512 != 0)
    {
        msg.push_back(0x00);
    }

    // Adds length of message to the end, makes the final 512
    for (int i = 7; i >= 0; --i)
    {
        msg.push_back(static_cast<uint8_t>((L >> (i * 8)) & 0xFF));
    }
...
std::vector<std::string> inputs = {"According", "to", "all", "known", "laws", "of", "aviation,", "there", "is", "no", "way", "a", "bee", "should", "be", "able", "to", "fly.",
